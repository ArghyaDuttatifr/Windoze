...
for i in range(n):
    ...
    x = np.random.uniform( 0, 1, dim )    # volume of a 10-D sphere: x1^2+x2^2+x3^2+x4^2+x5^2+x6^2+x7^2+x8^2+x9^2+x10^2 < R^2
    sq_R = 0
    for j in range(dim):
    ...
    if dR <= 1:        
        Zn += 1 
Vol = 2** dim *Zn /n    #dim = 2 and dim = 10         
